...
def create_post(post: PostCreate, db: Session = Depends(get_db)):
    ...
    #Createss a new post using the user_id from the request body.Sets it to None if the post is anonymous.

    # Convert string category to enum value
    category_value = post.category.value if hasattr(post.category, 'value') else post.category
...
def get_posts(
    ...
    return query.order_by(Post.timestamp.desc()).offset(skip).limit(limit).all()
