...
from pydantic_schemas.consultant_message import ConsultantMessageCreate, ConsultantMessageResponse, ConsultantMessageUpdate
...
def send_message(
    ...
    if message.receiver_type.value == "user":
        ...
        if not db.query(Consultant).filter(Consultant.id == message.receiver_id).first():
...
def get_message_thread(
    ...
    messages = db.query(ConsultantMessage).filter(
        (
    ...
    ).order_by(ConsultantMessage.timestamp).all()
...
def get_user_consultations(
    ...
    consultant_messages = db.query(ConsultantMessage).filter(
        (
            (ConsultantMessage.sender_id == user_id) & 
            (ConsultantMessage.sender_type == EntityType.user) &
            (ConsultantMessage.receiver_type == EntityType.consultant)
    ...
    for consultant_id in consultant_ids:
        ...
        last_message = db.query(ConsultantMessage).filter(
            (
                (ConsultantMessage.sender_id == user_id) & 
                (ConsultantMessage.sender_type == EntityType.user) &
                (ConsultantMessage.receiver_id == consultant_id) &
        ...
        ).order_by(ConsultantMessage.timestamp.desc()).first()
        
        # Count unread messages
        unread_count = db.query(ConsultantMessage).filter(
            (ConsultantMessage.sender_id == consultant_id) &
            (ConsultantMessage.sender_type == EntityType.consultant) &
            (ConsultantMessage.receiver_id == user_id) &
            (ConsultantMessage.receiver_type == EntityType.user) &
...
def get_consultant_clients(
    ...
    user_messages = db.query(ConsultantMessage).filter(
        (
            (ConsultantMessage.sender_id == consultant_id) & 
            (ConsultantMessage.sender_type == EntityType.consultant) &
            (ConsultantMessage.receiver_type == EntityType.user)
...
def update_message_status(message_id: str, status_update: ConsultantMessageUpdate, db: Session = Depends(get_db)):
    """
    Update the status of a message (mark as delivered or read)
    """
    message = db.query(ConsultantMessage).filter(ConsultantMessage.id == message_id).first()
    if not message:
