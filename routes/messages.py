...
def send_message(message: MessageCreate, db: Session = Depends(get_db), sender_id: str = None):
    ...
    if message.post_id:
        ...
        if post.category.value != "vent":
            raise HTTPException(status_code=400, detail="Messages can only be linked to vent posts")
...
def get_conversations(user_id: str, db: Session = Depends(get_db)):
    ...
    #Get all conversations for a specific user (people they've messaged orviceversa)
    
    # Find all distinct users this user has conversations
...
def get_message_thread(user_id: str, partner_id: str, db: Session = Depends(get_db)):
    ...
    #Getting all messages bn two users
...
def update_message_status(message_id: str, status_update: MessageUpdate, db: Session = Depends(get_db)):
    
    #Updatingg thestatus of a message (mark as delivered or read)
...
def get_vent_outreach_messages(post_id: str, db: Session = Depends(get_db)):
    ...
    if post.category.value != "vent":
        raise HTTPException(status_code=400, detail="Only vent posts can have outreach messages")
    
    messages = db.query(Message).filter(Message.post_id == post_id).order_by(Message.timestamp).all()
    return messages 